#!/usr/bin/python3
"""
Clone all of my vim plugins.

Based on https://github.com/mgedmin/cloneall.
"""
...
class RepoWrangler(object):
    ...
    def list_repos(self, owner, pattern):
        message = "Fetching list of {}'s repositories from GitHub...".format(owner)
        ...
        return sorted((r for r in repos if fnmatch.fnmatch(r['name'], pattern)),
                      key=itemgetter('name'))
    ...
    def call(self, args, **kwargs):
        ...
        if output:
            ...
            self.progress_item.error_info(
                '{command} exited with {rc}'.format(command=self.pretty_command(args), rc=retcode))
        return retcode
...
def main():
    parser = argparse.ArgumentParser(
        description="Clone/update all user repositories from GitHub")
    ...
    parser.add_argument('--user', default=DEFAULT_OWNER,
                        help='specify the GitHub user (default: %s)' % DEFAULT_OWNER)
    parser.add_argument('--pattern', default=DEFAULT_PATTERN,
                        help='specify repository name pattern (default: %s)' % DEFAULT_PATTERN)
    ...
    with Progress() as progress:
        ...
        repos = wrangler.list_repos(args.user, args.pattern)
        ...
        queue.finish()
        progress.finish(
            "{0.n_repos} repositories: {0.n_updated} updated, {0.n_new} new, {0.n_dirty} dirty.".format(wrangler))
